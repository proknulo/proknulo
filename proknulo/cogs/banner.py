...
#         guild = self.bot.get_guild(1057621071035117668)
#         with Image.open("banner.png") as img:
#             draw = ImageDraw.Draw(img)
#             font = ImageFont.truetype("SAIBA-45-Regular.ttf", 40)
#             total = len(guild.members)
#             voice = len([m for m in guild.members if m.voice])
#             if (total, voice) != self.last:
#                 x =765
#                 y =617
#                 draw.text(xy:(x, y), str(voice), font=font)
#
#                 x=1002
#                 y=569
#                 draw.text(xy:(x, y), str(total), font=font)
...
class Banner(commands.Cog):
    ...
    async def update_banner(self):
        ...
        with Image.open("banner.png") as img:
            ...
            font = ImageFont.truetype("SAIBA-45-Regular(v1.1).ttf", 40)
            total = len(guild.members)
            voice = len([m for m in guild.members if m.voice])
            if (total, voice) != self.last:
                x = 1002
